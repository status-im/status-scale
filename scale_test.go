...
func (s *WhisperScaleSuite) SetupTest() {
	...
	s.Require().NoError(err)
	s.p = project.New(
		filepath.Join(cwd, "wnode-test-cluster", "docker-compose.yml"),
		"wnodetestcluster",
		cli)
...
func (s *WhisperScaleSuite) TestSymKeyMessaging() {
	...
	s.InDelta(3.5, reports.MeanOldPerNew(), 0.8, "duplicates per new envelopes ratio is different from expected")
